...
//  }
        ...
        epoch_num GetEpoch(){return epoch_;}
    ...
    struct masstree_params : public Masstree::nodeparams<> {
        typedef storage::Tile  *value_type;
...
//  inline bool search(const key_type &k, storage::Tile &o, epoch_num e,
//                     versioned_node_t *search_info = nullptr) const;
        inline storage::Tile *search(const key_type &k, epoch_num e,
                                     versioned_node_t *search_info = nullptr) const;

        inline void search_amac(std::vector<AMACState> &states, epoch_num epoch) const;

        inline bool
        search_masstree(const key_type &k, storage::Tile &o, threadinfo &ti,
                        versioned_node_t *search_info = nullptr) const;
        ...
        public:
             ...
             */
            // above function is to find the OID versions, and this call the
            // value location, since the search method handle the value location
            // should we deal this here
            virtual bool invoke(const mbtree<masstree_params> *btr_ptr,
                                const string_type &k, storage::Tile *v,
        ...
        void search_range_call(const key_type &lower, const key_type *upper,
                               ...
                               concurrency::TransactionContext *xc) const;
        ...
        inline void search_range(const key_type &lower, const key_type *upper,
                                 F &callback, concurrency::TransactionContext *xc) const;
         ...
         */
        inline bool insert(const key_type &k, storage::Tile *o, concurrency::TransactionContext *xc,
         ...
         */
        inline bool insert_if_absent(const key_type &k, storage::Tile *o, concurrency::TransactionContext &xc,
                                     insert_info_t *insert_info = NULL);
         ...
         * is written into old_v
         */
        inline bool remove(const key_type &k, concurrency::TransactionContext *xc,
                           storage::Tile **old_v = NULL);
...
//  TableDescriptor *table_descriptor_;
    ...
    public:
        ...
        public:
            ...
            concurrency::TransactionContext *xc_;
        ...
        public:
            ScanIterator(concurrency::TransactionContext *xc, mbtree<P> *btr, const key_type &lower,
            ...
            storage::Tile *value() const { return sinfo_.entry.value(); }
...
//   oid_array *pdest_array() const { return btr_->pdest_array_; }
            ...
            static ScanIterator<IsReverse> factory(
                    mbtree<P> *mbtree,
                    concurrency::TransactionContext *xc,
        ...
        template <bool IsReverse>
        class peloton_masstree_ScanIterator {
        public:
            Masstree::scan_info<masstree_params> sinfo_;
            low_level_iterator_scanner<IsReverse> scanner_;
            concurrency::TransactionContext *xc_;
        ...
        public:
            peloton_masstree_ScanIterator(concurrency::TransactionContext *xc, mbtree<P> *btr, const key_type &lower,
                                          const key_type *upper)
                    : sinfo_(btr->get_table(), lower.operator Masstree::Str()),
                      scanner_(btr, upper),
                      xc_(xc),
                      btr_(btr) {}
            int count() const { return scancount_; }
...
//    oid_array *pdest_array() const { return btr_->pdest_array_; }
            ...
            bool init(bool emit_firstkey=true) {
                threadinfo ti(this->xc_->GetEpochId());
                ...
                while (1) {
                    {
                        ...
                        find_initial_retry_root:
                        {
                            ...
                            while (!v[sense].isleaf()) {
                                ...
                                int kp_ = Masstree::internode<P>::bound_type::upper(si.ka, *in);
                                n[!sense] = in->child_[kp_];
...
//      si.state = AWAIT si.stack[si.stackpos].find_initial(scan_iterator.helper_, si.ka, emit_firstkey, si.entry, ti);
                ...
                while (1) {
                    switch (si.state) {
                        ...
                        retry:
                            si.state = si.stack[si.stackpos].find_retry(this->helper_, si.ka, ti);
                            break;
                    }
                }
                return true;
...
//template <typename P>
//inline bool mbtree<P>::search(const key_type &k, storage::Tile &o, epoch_num e,
...
//}
    template <typename P>
    inline storage::Tile *mbtree<P>::search(const key_type &k, epoch_num e,
                                            versioned_node_t *search_info) const {
        threadinfo ti(e);
        Masstree::unlocked_tcursor<P> lp(table_, k.data(), k.size());
        bool found = lp.find_unlocked(ti);
                ...
                switch (s.stage) {
                            ...
                            if (likely(!in->has_changed(s.v[s.sense]))) {
                                ...
                                if (oldv.has_split(s.v[s.sense]) && in->stable_last_key_compare(s.lp.ka_, s.v[s.sense], ti) > 0) {
                                    ...
                                    if (s.v[s.sense].isleaf()) {
                                        ...
                                        Masstree::internode<P>* in_ = (Masstree::internode<P>*)s.n[s.sense];
                                        in_->prefetch();
                                        s.ptr = in_;
                                        assert(s.stage == 1);
                                    }
                    ...
                    stage0:
                        ...
                        if (s.v[s.sense].isleaf()) {
                            ...
                            auto *in_ = (Masstree::internode<P>*)s.n[s.sense];
                            in_->prefetch();
                            s.ptr = in_;
                            s.stage = 1;
                        }
...
// Multi-key search using Coroutines
    template <typename P>
    inline bool
    mbtree<P>::search_masstree(const key_type &k, storage::Tile &o, threadinfo &ti,
                               versioned_node_t *search_info) const {
        Masstree::unlocked_tcursor<P> lp(table_, k.data(), k.size());
    ...
    template <typename P>
    inline bool mbtree<P>::insert(const key_type &k, storage::Tile *o, concurrency::TransactionContext *xc,
    ...
    template <typename P>
    inline bool mbtree<P>::insert_if_absent(const key_type &k, storage::Tile *o,
                                            concurrency::TransactionContext &xc,
                                            insert_info_t *insert_info) {
        // Recovery will give a null xc, use epoch 0 for the memory allocated
        epoch_num e = xc.GetEpochId();
        PELOTON_ASSERT(e >= 0) ;

        threadinfo ti(e);
        Masstree::tcursor<P> lp(table_, k.data(), k.size());
//        LOG_DEBUG("key info, key size, %s,%u",k.GetInfo().c_str(),k.size());
        bool found =  lp.find_insert(ti);
        if (!found) {
            insert_new:
            found = false;
            ti.advance_timestamp(lp.node_timestamp());
            lp.value() = o;
//            LOG_DEBUG("value:location, tilegroupid, %p,%u",o,o->GetTileGroupId());
            if (insert_info) {
                insert_info->node = lp.node();
                insert_info->old_version = lp.previous_full_version_value();
                insert_info->new_version = lp.next_full_version_value(1);
 ...
 */
    template <typename P>
    inline bool mbtree<P>::remove(const key_type &k, concurrency::TransactionContext *xc,
                                  storage::Tile **old_v) {
        threadinfo ti(xc->GetEpochId());
        Masstree::tcursor<P> lp(table_, k.data(), k.size());
...
//    *old_v = oidmgr->oid_get_latest_version(tuple_array_, lp.value());
    ...
    public:
        ...
        void check(const Masstree::scanstackelt<P> &iter,
            ...
            int min = std::min((uint32_t)boundary_->size(), key.prefix_length());
    ...
    public:
        ...
        inline bool visit_value(const Masstree::key<uint64_t> &key, storage::Tile *value) {
            if (this->boundary_compar_) {
                lcdf::Str bs(this->boundary_->data(), this->boundary_->size());
                if ((!Reverse && bs <= key.full_string()) ||
                    (Reverse && bs >= key.full_string()))
    ...
    public:
        ...
        bool invoke(const string_type &k, storage::Tile *o, storage::Tile *v, const node_opaque_t *n,
