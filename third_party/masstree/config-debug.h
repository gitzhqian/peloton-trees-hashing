...
#define masstree_invariant(x, ...) do { if (!(x)) {fprintf(stderr,"invariant error;");abort();} } while (0)
#else
...
#define masstree_precondition(x, ...) do { if (!(x)) {fprintf(stderr,"precondition error;");abort();} } while (0)
#else
...
//    __attribute__((noreturn));
#define always_assert(x, ...) do { if (!(x)) {fprintf(stderr,"assert error;");abort();} } while (0)
