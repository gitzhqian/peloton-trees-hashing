...
template <typename T>
static inline std::string Encode(const T &t) {
  const encoder<T> enc;
  return enc.write(&t);
}

template <typename T>
static inline const char *Encode(uint8_t *buf, const T &t) {
  const encoder<T> enc;
...
template <typename T>
static inline const T *Decode(const std::string &buf, T &obj) {
  const encoder<T> enc;
  return enc.read(buf.data(), &obj);
}

template <typename T>
static inline const T *Decode(const char *buf, T &obj) {
  const encoder<T> enc;
...
template <typename T>
static inline const T *PrefixDecode(const std::string &buf, T &obj,
                                    size_t prefix) {
  const encoder<T> enc;
...
template <typename T>
static inline const T *PrefixDecode(const char *buf, T &obj, size_t prefix) {
  const encoder<T> enc;
  return enc.read(buf, &obj, prefix);
}

template <typename T>
static inline size_t Size(const T &t) {
  const encoder<T> enc;
  return enc.nbytes(&t);
}
...
#define STRUCT_LAYOUT_X(tpe, name) tpe name;
...
#define STRUCT_INITLIST_REST_X(tpe, name) , name(name)

#define STRUCT_PRINTER_FIRST_X(tpe, name) #name << "=" << obj.name

#define STRUCT_PRINTER_REST_X(tpe, name) << ", " << #name << "=" << obj.name

#define STRUCT_FIELDPOS_X(tpe, name) name##_field,
...
#define DESCRIPTOR_VALUE_MAX_NBYTES_X(tpe, name) \
  serializer<tpe, true>::max_nbytes(),
#define DESCRIPTOR_VALUE_OFFSETOF_X(tpe, name) offsetof(value, name),
#define DESCRIPTOR_VALUE_SIZEOF_X(tpe, name) sizeof(tpe),
...
#define DO_STRUCT_COMMON(name)                                                \
  inline std::string &write(std::string &buf, const struct name *obj) const { \
    buf.clear();                                                              \
    buf.resize(nbytes(obj));                                                  \
    write((uint8_t *)buf.data(), obj);                                        \
    return buf;                                                               \
  }                                                                           \
  inline std::string write(const struct name *obj) const {                    \
    std::string ret;                                                          \
    write(ret, obj);                                                          \
    return ret;                                                               \
  }                                                                           \
...
#define DO_STRUCT(name, keyfields, valuefields)                                \
  struct name {                                                                \
    ...
    struct value_descriptor {                                                  \
      ...
      static inline constexpr size_t nfields() {                               \
        return static_cast<size_t>(value::NFIELDS);                            \
      }                                                                        \
      static inline size_t max_nbytes(size_t i) {                              \
        static size_t maxn[] = {                                               \
            APPLY_X_AND_Y(valuefields, DESCRIPTOR_VALUE_MAX_NBYTES_X)};        \
        return maxn[i];                                                        \
      }                                                                        \
      static inline size_t cstruct_offsetof(size_t i) {                        \
        static size_t offsets[] = {                                            \
            APPLY_X_AND_Y(valuefields, DESCRIPTOR_VALUE_OFFSETOF_X)};          \
        return offsets[i];                                                     \
      }                                                                        \
      static inline size_t cstruct_sizeof(size_t i) {                          \
  ...
  inline std::ostream &operator<<(std::ostream &o, const name::key &obj) {     \
    o << "{" << keyfields(STRUCT_PRINTER_FIRST_X, STRUCT_PRINTER_REST_X)       \
      << "}";                                                                  \
    return o;                                                                  \
  }                                                                            \
  inline std::ostream &operator<<(std::ostream &o, const name::value &obj) {   \
  ...
  struct encoder<name::key> {                                                  \
    inline void encode_write(uint8_t *buf,                                     \
                             const struct name::key *obj) const {              \
      APPLY_X_AND_Y(keyfields, SERIALIZE_WRITE_KEY_FIELD_X)                    \
    }                                                                          \
    inline void encode_read(const uint8_t *buf, struct name::key *obj) const { \
      APPLY_X_AND_Y(keyfields, SERIALIZE_READ_KEY_FIELD_X)                     \
    }                                                                          \
    inline void encode_prefix_read(const uint8_t *buf, struct name::key *obj,  \
                                   size_t prefix) const {                      \
      size_t i = 0;                                                            \
      APPLY_X_AND_Y(keyfields, SERIALIZE_PREFIX_READ_KEY_FIELD_X)              \
    }                                                                          \
    inline bool encode_failsafe_read(const uint8_t *buf, size_t nbytes,        \
                                     struct name::key *obj) const {            \
    ...
    static inline constexpr size_t encode_max_nbytes() {                       \
  ...
  struct encoder<name::value> {                                                \
    inline void encode_write(uint8_t *buf,                                     \
                             const struct name::value *obj) const {            \
      APPLY_X_AND_Y(valuefields, SERIALIZE_WRITE_VALUE_FIELD_X)                \
    }                                                                          \
    inline void encode_read(const uint8_t *buf,                                \
                            struct name::value *obj) const {                   \
      APPLY_X_AND_Y(valuefields, SERIALIZE_READ_VALUE_FIELD_X)                 \
    }                                                                          \
    inline void encode_prefix_read(const uint8_t *buf,                         \
                                   struct name::value *obj,                    \
                                   size_t prefix) const {                      \
      size_t i = 0;                                                            \
    ...
    inline size_t encode_nbytes(const struct name::value *obj) const {         \
      size_t size = 0;                                                         \
